...
df['예측일사_kWh'] = df['예측일사_MJ'] * MJ_to_kWh
df['예상_발전량_kWh'] = df['예측일사_kWh'] * area * num * efficiency
df
df.head()
df['절약요금'] = 0  # Initialize the column
df['일시'].dt.date.unique()
# Apply the conditions
df.loc[(df['hour'] == 11) | ((df['hour'] > 12) & (df['hour'] < 18)), '절약요금'] = df['예상_발전량_kWh'] * 229
df.loc[((df['hour'] > 21) | (df['hour'] < 8)), '절약요금'] = df['예상_발전량_kWh'] * 94
df.loc[((df['hour'] > 7) & (df['hour'] < 11)) | (df['hour'] == 12) | ((df['hour'] > 17) & (df['hour'] < 22)), '절약요금'] = df['예상_발전량_kWh'] * 146.9

building = df.groupby(['num'],as_index=False).agg(총_전력사용량_kWh=('전력사용량_kWh','sum'),
